...
import { useDeleteClassWiseSubjectEntryMutation } from '../../../redux/API/setup/school/classWiseSubjectEntryAPI';

const ClassWiseSubjectEntryTable = ({ fetchedData, setEdit, setShowFormPage, setFormData }) => {
  ...
  const maxSubject = Math.max(...fetchedData?.map(item => item.subject.length));
  ...
  const [deleteClassWiseSubjectEntry, { isError, isSuccess, error }] = useDeleteClassWiseSubjectEntryMutation();
  ...
  const handleDelete = (id) => {
    deleteClassWiseSubjectEntry({ id })
  }
  ...
  const handleEdit = (item) => {
    let updateSubject = item.subject.map((data, index) => {
      return {
        _id: data._id._id,
        name: data._id.name,
        marks: data.marks,
    ...
    setFormData({
      _id: item._id, company: item.company._id, branch: item.branch._id, classes: item.class._id, remarks: item.remarks, subject: updateSubject
    })
    setEdit(true);
        ...
        <table className=" w-[100%]  table-auto border-collapse border border-gray-300 text-left my-2 whitespace-nowrap">
          <thead>
            <tr className=' text-center'>
              ...
              {Array.from({ length: maxSubject }).map((_, index) => {
                return <React.Fragment key={index}>
                  <th className="  border border-gray-300 px-2 py-1">Subject Name {index + 1}</th>
                  <th className="  border border-gray-300 px-2 py-1">Subject Marks {index + 1}</th>
                </React.Fragment>
              })}
          ...
          <tbody className='text-center'>
            {
              fetchedData?.slice(startIndex, endIndex)?.reverse()?.map((item, i) => (
                <tr className="bg-blue-100 odd:bg-blue-200" key={i}>
                  ...
                  {Array.from({ length: maxSubject }).map((_, index) => {
                    return <React.Fragment key={index}>
                      <td className="border border-gray-300 px-2 py-2">{item?.subject[index]?._id?.name ? item?.subject[index]?._id?.name : "-"}</td>
                      <td className="border border-gray-300 px-2 py-2">{item?.subject[index]?.marks !== undefined ? item?.subject[index]?.marks : "-"}</td>
                    </React.Fragment>
                  })}
...
export default ClassWiseSubjectEntryTable