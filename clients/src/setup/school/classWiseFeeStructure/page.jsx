...
import ClassWiseFeeStructureForm from './ClassWiseFeeStructureForm'
import ClassWiseFeeStructureTable from './ClassWiseFeeStructureTable'
import { useGetClassWiseFeeStructureQuery } from '../../../redux/API/setup/school/classWIseFeeStructureAPI';
...
const ClassWiseFeeStructure = () => {
    ...
    const [formData, setFormData] = useState({
        _id: "", company: "", branch: "", classes: "", remarks: "", fee: []
    })
    const { data, isError, isSuccess } = useGetClassWiseFeeStructureQuery();

    useEffect(() => {
        if (data) {
            setFetchedData(data.classWiseFeeStructure);
            console.log(data.classWiseFeeStructure)
        } else {
            setFetchedData([])
        }
        if (isError) {
    ...
    return (
        <BaseLayout>
            {!showFormPage ? <div className='ms-2 xl:me-24 me-4 my-4 flex flex-col justify-center  '>
                <h2 className=' heading pageHeading '>Class Wise Fee Structure</h2>
                ...
                <div className=''>
                    <ClassWiseFeeStructureTable fetchedData={fetchedData} setEdit={setEdit} setFormData={setFormData} setShowFormPage={setShowFormPage} />
                </div>
            </div> : <ClassWiseFeeStructureForm setShowFormPage={setShowFormPage} formData={formData}
