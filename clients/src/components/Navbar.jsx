...
const Navbar = () => {
    ...
    const [toggleMenuItem, setToggleMenuItem] = useState(null)
    const [toggleSubMenuItem, setToggleSubMenuItem] = useState(null)
        ...
        <>
            ...
            <div className=' md:hidden    '>
                <div className='flex flex-row  absolute'>
                    {menu &&
                        <ul className={`inline-flex flex-col w-60 h-[80vh] pt-6   bg-blue-100 py-1 md:hidden z-10 ${menu}?'transition animate-back-forth duration-5000  ease-in-out':''`}>

                            {SIDENAV_ITEMS.map((item, key) => (
                                <li className="relative group px-2 py-1" key={key}>
                                    {/* Main Menu Item */}
                                    <Link src={item.path} className="flex flex-row" >
                                        <span className="p-1" onClick={() => setToggleMenuItem(toggleMenuItem === key ? null : key)}>
                                            <VscTriangleRight />
                                        </span>
                                        <span>{item.title}</span>
                                    ...
                                    {/* Submenu */}
                                    {toggleMenuItem === key && item?.submenu && (
                    ...
                    <div className={` mx-4   mt-4 z-100  ${menu}?' ':' ' `}>
